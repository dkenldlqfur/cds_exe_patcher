...
class SaveSlotSelectorTests(unittest.TestCase):
    def test_install_creates_ten_two_digit_destinations_and_restores_cleanly(self) -> None:
        ...
        self.assertEqual(_relative_target(wrapper, wrapper_va, 0x12D), SAVE_SLOT_SELECTOR_CONFIRM_ROUTINE_VA)
        self.assertEqual(_relative_target(wrapper, wrapper_va, 0x157), SAVE_SLOT_SELECTOR_SUCCESS_VA)
        self.assertEqual(_relative_target(wrapper, wrapper_va, 0x174), SAVE_SLOT_SELECTOR_CANCEL_VA)
        self.assertEqual(
            wrapper_va + 0x13A + struct.unpack_from("b", wrapper, 0x139)[0],
            wrapper_va + 0xBB,
        )
