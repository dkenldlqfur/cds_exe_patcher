...
실행 파일의 확인 근거와 알려진 제약은 [EXE 분석 기록](docs/EXE_ANALYSIS.md)에서 관리합니다.
...
- 좌표 표시 형식 변경
- 장기 휴양, 탐험 준비, 망원경의 도시·항구 발견 시야 보정, 세대교체 및 인물 활동 가능 나이 설정
- 소지금·저금 공통 상한과 주인공 명성·악명 공통 상한 설정
- 체력·지력·무력·매력·운·신앙심의 공통 상한(0~255)과 생명력 상한(0~9999) 설정
- 일반 NPC의 월간·일일 이동 판정, 출발 확률과 도착 대기 기간 설정
...
- `DISCOVER.CDS`의 발견 애니메이션 29종을 내장 Cinepak AVI로 전환(백과사전 삽화와 실제 발견 이벤트 동시 적용)
- 카바신전 발견물, 정지 이미지와 이벤트 주입
- 노예 도서관 힌트와 발견 대사 추가
- 무제국 힌트 획득 후 발견 가능하도록 조건 수정
- 인도·향료제도·중국·지팡그 발견 이벤트에 정지 이미지 추가
- 해상괴물 전투에서 퇴각했을 때 조우가 완료 처리되는 버그 수정
- 매각한 선박의 슬롯을 재사용할 때 기존 함포 중량이 새 선박에 더해지는 버그 수정
- 도크 후 선박 구입 목록의 빈 슬롯을 클릭하면 게임이 종료되는 버그 수정
- 크노소스 주점 힌트 대상 코드와 카카오 주점 힌트 제공 도시를 함께 수정
- 모뉴멘트밸리 원주민 교섭의 언어 판정을 아프리카토착어에서 중남미토착어로 수정
- HIST_EV의 발견 후 경과 연수를 `현재 연도 - 발견 연도`로 정상 판정
- 각 버그 수정 항목을 독립적으로 선택하고 적용 상태를 확인 가능
...
- 인물: 기본 정보, 등용 상태, 생명력(0~9999), 능력치(각 0~255), 기술과 언어
- 함선: 선종 이름, 조선소 조건, 추진력·내구력·적재량·대포·승무원 수치
- 병종: 병종별 육상전 공격·방어 계산식의 기술·능력치 계수와 상수
- 도시: 기본 정보, 조선소, 시장, 교역, 시설·내륙 연결과 도시별 시설 선택 영역
- 교역권: 교역권별 공통 교역품 5종
- 아이템: 이름, 분류, 매입가·매각가, 효과 코드
- 발견물: 이름, 분류, 가치, 발견 좌표 범위와 미디어 연결
- 힌트: 힌트 186개의 이름·대상 ID·요구 기술·언어·레벨·선행 발견물 편집, 연결 정보 조회와 도서관 책 제목·저자·출현 연도·도시 편집 (힌트 이름·책 제목·저자는 한글 18자/CP949 36바이트)
